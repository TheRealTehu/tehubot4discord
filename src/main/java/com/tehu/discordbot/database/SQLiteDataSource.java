...
public class SQLiteDataSource implements DatabaseManager {
    ...
    public SQLiteDataSource() {
        ...
        try (final Statement statement = getConnection().createStatement()) {
            // language=SQLite
            String defaultPrefix = Config.get("prefix");
    ...
    private void createOrChooseFile() {
        try {
    ...
    @Override
    public boolean containsGuild(long guildId) {
        final String sql = "SELECT id FROM guild_settings WHERE guild_id = ?";

        try (Connection connection = getConnection()) {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, String.valueOf(guildId));
